...
import { tokenDataSchema } from '../schemas/auth-api.schemas';
import type {
	LoginFormData,
	LoginResponse,
	RefreshResponse,
...
export const authApi = {
	...
	refresh: async (): Promise<TokenData> => {
		try {
			const response = await apiClient.post<RefreshResponse>(API_CONFIG.ENDPOINTS.REFRESH);
 ...
 */
export function isTokenForCorrectAudience(
	token: string,
	expectedAudience: string = 'frontend-app'
): boolean {
	const payload = decodeTokenPayload(token);
